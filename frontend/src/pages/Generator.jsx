...
const App = () => {
  ...
  async function generateBlogContent() {
    ...
    try {
      const response = await fetch(`${import.meta.env.VITE_BACKEND_URL}/api/blog/generate`, {
        ...
        body: JSON.stringify({
          topic: text,
          links: links.filter(link => link),  // Optional reference links
        }),
      });
      ...
      // Optional: insert uploaded image into blog
      if (imageURL) {
  ...
  const handleSaveBlog = async () => {
    ...
    try {
      const res = await fetch(`${import.meta.env.VITE_BACKEND_URL}/api/blogs/save`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${localStorage.getItem("token")}`,  // ✅ Right here
        },
